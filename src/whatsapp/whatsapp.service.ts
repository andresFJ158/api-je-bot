...
export class WhatsAppService implements OnModuleInit {
  ...
  private async initializeWhatsApp() {
    ...
    try {
      ...
      // ═══════════════════════════════════════════════════════════════════
      // ⚠️ CRÍTICO: Listen for contacts.upsert to get real phone numbers for LIDs
      // Este es el ÚNICO evento donde WhatsApp envía el número real relacionado con el LID
      // Sin este listener, phone siempre será null
      // ═══════════════════════════════════════════════════════════════════
      this.logger.log('═══════════════════════════════════════════════════════════════════');
      this.logger.log('📞 REGISTERING contacts.upsert LISTENER');
      this.logger.log('⚠️  THIS IS CRITICAL: This is the ONLY event where WhatsApp sends real phone numbers');
      this.logger.log('═══════════════════════════════════════════════════════════════════');
      
      this.socket.ev.on('contacts.upsert', async (contacts) => {
        this.logger.log('🔔🔔🔔 contacts.upsert EVENT RECEIVED! Processing contacts...');
        await this.handleContactsUpsert(contacts);
      });
      
      this.logger.log('✅✅✅ contacts.upsert listener registered successfully');
      this.logger.log('═══════════════════════════════════════════════════════════════════');
   ...
   * 
   * ⚠️ ÚNICA FORMA REAL DE QUE SE GUARDE EL NÚMERO
   * Este es el ÚNICO evento donde WhatsApp envía el número real relacionado con el LID
   * 
  ...
  private async handleContactsUpsert(contacts: any[]): Promise<void> {
    // Log crítico para confirmar que el método se está ejecutando
    this.logger.log('═══════════════════════════════════════════════════════════════════');
    this.logger.log('🔔 handleContactsUpsert CALLED - This is where real phone numbers arrive!');
    this.logger.log('═══════════════════════════════════════════════════════════════════');
    
    if (!contacts || contacts.length === 0) {
      this.logger.warn('[handleContactsUpsert] ⚠️ No contacts received or empty array');
      return;
    }
    ...
    // ═══════════════════════════════════════════════════════════════════
    // PATRÓN EXACTO: Iterar sobre contactos y actualizar por LID
    // ═══════════════════════════════════════════════════════════════════
    for (const c of contacts) {
      try {
        // Verificar que tenga id con @s.whatsapp.net Y que tenga lid
        if (c.id?.endsWith('@s.whatsapp.net') && c.lid) {
          // Extraer el número simplemente removiendo @s.whatsapp.net
          const phone = c.id.replace('@s.whatsapp.net', '');

          this.logger.log('📞 Número recibido:', phone, 'para', c.lid);

          // Actualizar todos los usuarios que tengan este LID en whatsappJid
          const result = await this.prisma.user.updateMany({
            ...
            data: {
              phone: phone,
              whatsappJid: c.id, // También actualizar el JID al real
            },
          });

          if (result.count > 0) {
            this.logger.log(`✅ Updated ${result.count} user(s) with phone: ${phone} for LID: ${c.lid}`);
          } else {
            this.logger.warn(`⚠️ No users found with LID: ${c.lid} - contact may not have sent a message yet`);
          }
        } else {
          this.logger.debug(`[handleContactsUpsert] Skipping contact - missing id (@s.whatsapp.net) or lid:`, {
            hasId: !!c.id,
            idEndsWith: c.id?.endsWith('@s.whatsapp.net'),
            hasLid: !!c.lid,
          });
        }
      } catch (error) {
